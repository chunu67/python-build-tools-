#!/usr/bin/perl
#
# Originally dpkg-shlibdeps.pl (from dpkg)
# Trimmed down by Rob so that it only dumps the dependencies to stdout.
# I'd prefer to have this in pure Python, but all the important Dpkg stuff is in perl.
#
# ORIGINAL CODE:
...
#
# UGLY HACK CODE FOR PYTHON-BUILD-TOOLS:
#  Copyright © 2015 Rob "N3X15" Nelson <nexisentertainment@gmail.com>
#
...
if ( -d 'debian' ) {
    ...
    my %uniq =
      map { guess_pkg_root_dir($_) => 1 } ( @pkg_symbols, @pkg_shlibs );
...
foreach (@ARGV) {
    if (m/^-T(.*)$/) {
        ...
        if ( not defined $depstrength{$dependencyfield} ) {
            warning( _g("unrecognized dependency field '%s'"),
                $dependencyfield );
        }
    }
    elsif (m/^-e(.*)$/) {
...
foreach my $file ( keys %exec ) {
    ...
    foreach my $soname (@sonames) {
        ...
        unless ( defined $lib ) {
            ...
            if ( scalar( split_soname($soname) ) ) {
                ...
                warning( $msg, $soname, $file, $obj->{format},
                    join( ':', @{ $obj->{RPATH} } ) );
            }
            next;
        }
    ...
    foreach my $lib ( keys %libfiles ) {
        ...
        foreach my $pkg ( @{ $file2pkg->{$lib} } ) {
            ...
            my $haslocaldep = 0;
            if ( -e $shlibslocal
                and defined( extract_from_shlibs( $soname, $shlibslocal ) ) )
            {
                $haslocaldep = 1;
            }
            ...
            if ( defined($symfile_path) ) {
                ...
                $symfile->merge_object_from_symfile(
                    $symfile_cache{$symfile_path}, $soname );
            }
            if ( defined($symfile_path) && $symfile->has_object($soname) ) {
                ...
                if ( ( $id ne $soname ) and ( $id ne $lib ) ) {
                    warning( _g('%s has an unexpected SONAME (%s)'), $lib,
                        $id );
                ...
                if ( $libobj->is_public_library()
                    and not add_shlibs_dep( $soname, $pkg, $lib ) )
                {
    ...
    foreach my $sym ( $obj->get_undefined_dynamic_symbols() ) {
        ...
        if ( keys %symdep ) {
            my $depends =
              $symfile->get_dependency( $symdep{soname},
            ...
            if ( not defined($syminfo) ) {
                ...
                if ( $obj->is_executable() or $obj->is_public_library() ) {
                    ...
                    unless ( $sym->{weak} ) {
                        if (   $debug
                            or ( $in_public_dir and $nb_warnings < 10 )
...
'%s contains an unresolvable reference to '
    ...
    foreach my $soname (@sonames) {
        ...
        unless ( $soname_notfound{$soname} or $soname_used{$soname} ) {
            ...
            next
              if ( $soname =~ /^libm\.so\.\d+$/
                and any { m/^libstdc\+\+\.so\.\d+/ } @sonames );
...
foreach my $soname ( keys %global_soname_needed ) {
    unless ( $global_soname_notfound{$soname} or $global_soname_used{$soname} )
    {
        next
          if ( $soname =~ /^libm\.so\.\d+$/
            and any { m/^libstdc\+\+\.so\.\d+/ } keys %global_soname_needed );
...
if ( $error_count >= 1 ) {
    my $note =
      _g(   'Note: libraries are not searched in other binary packages '
    ...
    if ( not exists( $depseen{$dep} ) ) {
        ...
        if ( $depseen{$dep} eq $dependencies{$field}{$dep} ) {
            ...
            version_compare_relation(
                $depseen{$dep}, REL_GT, $dependencies{$field}{$dep}
            )
          )
        {
...
foreach my $field ( reverse @depfields ) {
    my $dep = '';
    if ( exists $dependencies{$field}
        and scalar keys %{ $dependencies{$field} } )
    {
        $dep = join ', ', map {
    ...
    if ($dep) {
        ...
        error( _g('invalid dependency got generated: %s'), $dep )
          unless defined $obj;
        $obj->sort();
...
sub update_dependency_version {
    ...
    foreach my $subdep ( split /\s*,\s*/, $dep ) {
        if ( exists $dependencies{$cur_field}{$subdep}
            and defined( $dependencies{$cur_field}{$subdep} ) )
        {
            if (
                $dependencies{$cur_field}{$subdep} eq ''
                or $minver ne '' and version_compare_relation(
                    $minver, REL_GT, $dependencies{$cur_field}{$subdep}
                )
              )
            {
...
sub find_packages {
    ...
    syserr( _g('cannot fork for %s'), 'dpkg-query --search' )
      unless defined $pid;
    if ( !$pid ) {
    ...
    while (<$dpkg_fh>) {
        ...
        if (m/^local diversion |^diversion by/) {
            ...
            my ( $pkgs, $path ) = ( $1, $2 );
            $cached_pkgmatch{$path} = $pkgmatch->{$path} =
              [ split /, /, $pkgs ];
        }
        else {
